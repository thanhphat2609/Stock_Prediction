...
# Connect to Hive	
spark = SparkSession.builder.appName("Transformation").config("spark.sql.warehouse.dir", "hdfs://localhost:9000/user/hive/warehouse").enableHiveSupport().getOrCreate()
...
input_path = 'hdfs://localhost:9000/user/thanhphat/datalake/year={}/month={}/day={}/part-*.csv'.format(year, month, date)
...
def sort_year(dataframe):
	...
	dataframe_new = dataframe.sort(dataframe.Year.asc()).drop("Year")
...
new_column_name = ['Date', 'Ajusting', 'Closing', 'Opening', 'High', 'Low']
...
# ========================================================================== STEP 4: EXPORT DATA FOR ENRICH ==========================================================================
df_vcb.coalesce(1).write.mode("overwrite").csv("/home/thanhphat/Downloads/", header = True)
